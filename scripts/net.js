...
let Net = function(ancestor, mutation = true, fullMutation = false) {
  this.weight = []
  this.bias = []
  ...
  if (!ancestor) {
    ...
    if (mutation) {
      ...
      if (fullMutation) {
        ...
        for (let i = 0; i < Math.round(rand(1, 5)); i++) {
          this.weight[Math.round(rand(0, 13))] += rand(-3, 3)
        }

        for (let i = 0; i < Math.round(rand(1, 5)); i++) {
          this.bias[Math.round(rand(0, 8))] += rand(-3, 3)
        }
      }
  ...
  this.output = input => {
    ...
    // let res =  this.neuron(this.weight[40], this.weight[41], this.bias[20], layer_5[0], layer_5[1])
    let res = layer_4[0]
    return res
  }
